...
import {  deleteAction,openAction} from "../redux/slices/taskSlice";
            ...
            <Box>
                  {
                    addNewTaskArr.map((item) => {
                      return(
<div key={item.id}>
  <Box sx={{ mt: 1, width: "94%", borderRadius: 1, backgroundColor: "white", height: "auto", cursor:"pointer"}}>
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <Box sx={{ display: "flex", alignItems: "center" }}>
        ...
        />
        <Typography sx={{ ml: 1, fontSize: 19 }}>{item.title}</Typography>
      </Box>

      <Box sx={{ mr: 2, display: "flex", alignItems: "center" }}>
        <CloseIcon sx={{ cursor: "pointer" }} onClick={() => handleDeleteAction(item.id)} />
      </Box>
    </Box>
    <Box sx={{ml:1}}>
      <Typography>{item.priority}</Typography>
                    ...
                    <Box onClick={handleClick} sx={{mt:1, width:"94%",cursor:"pointer", borderRadius:1, backgroundColor:"lightgrey", color:"grey", height:"35px",display:"flex",alignItems:"center" }}>
